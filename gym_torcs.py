...
class TorcsEnv:
    ...
    terminal_judge_start = 500     # If after 100 timestep still no progress, terminated
    ...
    def step(self, step, client, u, early_stop):
        ...
        #progress - Normal lanekeeping term
        #pure_reward - The reward function without any extra conditions 
        #(Pure_reward is same as progress for lanekeeping)
        #reward - the reward function with extra conditions
        progress = sp*np.cos(obs['angle'])-np.abs(sp*np.sin(obs['angle']))-(sp* np.abs(obs['trackPos']))     
        reward = progress
        pure_reward = progress 

        client.racepos = obs['racePos']
        client.best_vel(sp)
        ...
        # collision detection
        if self.terminal_judge_start < self.time_step:
             if obs['damage'] - obs_pre['damage'] > 10: #Parameter can be changed
                client.collided()
                reward = -1000
                episode_terminate = True
                client.R.d['meta'] = True
                print('Terminating because Collided hard')
        ...
        #Finally save the rewards for analysis later
        client.inc(pure_reward,reward,progress,sp)
        self.time_step += 1
    ...
    def agent_to_torcs(self, u):
        ...
        if self.throttle is True:  # throttle action is enabled, generally true
            torcs_action.update({'accel': u[1]})
            torcs_action.update({'brake': u[2]})

        if self.gear_change is True: # gear change action is enabled, generally false
