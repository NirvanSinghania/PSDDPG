...
class DDPG:
    ...
    def __init__(self, env_name, state_dim,action_dim,save_location):
        ...
        self.time_step = 0 
        #Added as an extra measure -> Nirvan
    ...
    def noise_action(self,state,epsilon):
        ...
        #print("Steering initial ",str(action[0]))
        noise_t[0] = epsilon * self.OU.function(action[0],  0.0 , 0.6, 0.8 )
        ...
        #action[0] = noise_t[0]
        #print("Steering final ", str(action[0]))
        action[0] = np.clip(action[0], -1, 1)
        action[1] = np.clip(action[1], 0 , 1)
        action[2] = np.clip(action[2], 0 , 1)
