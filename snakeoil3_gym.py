...
class Client(object):
    def __init__(self,H=None,p=None,i=None,e=None,t=None,s=None,d=None,vision=False):
        ...
        self.total_vel = 0
        self.pure_reward = 0
        self.avg_pure_reward_vel = 0
        self.avg_progress_vel = 0
        self.avg_reward_vel = 0
    ...
    def inc(self,pure_reward,reward,progress,velocity):
        self.pure_reward += pure_reward
        self.reward += reward
        self.progress += progress
        velocity = max(velocity,1)
        #Normalization with respect to velocity
        self.avg_reward_vel += (reward/velocity)
        self.avg_progress_vel += (progress/velocity)
        self.avg_pure_reward_vel += (pure_reward/velocity)
        self.num_steps += 1
    ...
    def trackleft(self):
        self.out_of_tracks += 1

    def exceeded_velocity(self):
        self.inc_vel += 1
    def best_vel(self,velocity):
        self.total_vel += velocity
        if velocity > self.maxvel:
            self.maxvel = velocity
    def turned_back(self):
        self.back += 1

    def printAnalysis(self,file, ep_no):
        tot_steps = self.num_steps
        tot_reward = self.reward
        tot_pure_reward = self.pure_reward
        tot_progress = self.progress
        avg_pure_reward = tot_pure_reward/tot_steps
        avg_reward = tot_reward/tot_steps
        avg_progress = tot_progress/tot_steps
        avg_collisions = self.num_collisions/tot_steps
        avg_out = self.out_of_tracks/tot_steps
        avg_inc = self.inc_vel/tot_steps
        avg_vel =  self.total_vel/tot_steps
        tot_turned_back = self.back
        avg_progress_vel = self.avg_progress_vel
        avg_pure_reward_vel  = self.avg_pure_reward_vel
        avg_reward_vel = self.avg_reward_vel
        #avg_reward_without_back = (tot_reward+100*tot_turned_back )/max(1,tot_steps- tot_turned_back)
        print(ep_no,tot_steps,tot_pure_reward,tot_reward,tot_progress,avg_pure_reward_vel, \
            avg_reward_vel,avg_progress_vel,avg_vel,avg_collisions,avg_out,avg_inc,self.maxvel, \
            self.racepos,tot_turned_back,sep=",",file=file)
...
def drive_example(c):
    ...
    correct thing to do is write your own `drive()` 
    function.'''
    S,R= c.S.d,c.R.d
    target_speed= 50
...
def drive_experiment(c):
    u'''This is an experiment function built on top of drive_example.'''
    S,R= c.S.d,c.R.d
    ...
    R[u'steer']= S[u'angle']*10/ PI
    # Stgeer To Center
    R[u'steer']-= S[u'trackPos']*.10*random.randint(1,2)
    ...
    if S[u'speedX'] < target_speed - (R[u'steer']*50):
        ...
        R[u'accel']-= .02*(c.x)
    if S[u'speedX']<4 and S[u'speedX']>1:
       R[u'accel']+= ((1/(S[u'speedX']+.1)))*0.05
